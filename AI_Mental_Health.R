...
funnel <- plot(sel, ylim=c(0,5))
funnel

png(file="countour_funnel_color.png")
...
data <- data.frame(
  Year = 2019:2025,
  Meta_analysis = c(0, 0, 1, 3, 1, 6, 3),
  Systematic_review = c(1, 0, 2, 3, 1, 17, 3)
)
...
library(tidyr)
data_long <- pivot_longer(data, cols = c(Meta_analysis, Systematic_review),
                          names_to = "Type", values_to = "Count")

# Plot
ggplot(data_long, aes(x = Year, y = Count, color = Type, group = Type)) +
  geom_line(size = 1.2) +
  geom_point(size = 3) +
  scale_color_manual(values = c("Meta_analysis" = "steelblue", "Systematic_review" = "firebrick")) +
  theme_minimal(base_size = 14) +
  labs(title = "Trends in Meta-analyses and Systematic Reviews (2019–2025)",
       x = "Year",
